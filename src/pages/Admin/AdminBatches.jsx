...
const AdminBatches = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`http://localhost:5000/admin/courses/${courseId}/batches`, { headers: authHeader() });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/admin/courses/${courseId}/batches/${id}`, { method: 'DELETE', headers: authHeader() });
